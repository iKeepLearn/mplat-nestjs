    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            let input = match input_fn(&req).await {
                Ok(input) => input,
                Err(e) => {
                    tracing::error!("Rate limiter input function failed: {e}");
            ...
            let (output, rollback) = match backend.request(input).await {
                ...
                Err(e) => {
                    if fail_open {
                        tracing::warn!("Rate limiter failed: {}, allowing the request anyway", e);
                        (None, None)
                    } else {
            ...
            if let Some(token) = rollback
                && let Some(rollback_condition) = rollback_condition
            {
