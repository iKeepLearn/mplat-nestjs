    ...
    O: Future<Output = Result<BI, actix_web::Error>>,
{
        ...
        BO: HeaderCompatibleOutput,
    {
        ...
        self.denied_response = Rc::new(|status| {
            ...
            let mut response = HttpResponse::TooManyRequests().json(ErrorResponse {
                status_code: StatusCode::TOO_MANY_REQUESTS.as_u16(),
                timestamp: Utc::now(),
                message: format!("操作太频繁了，请等待{}秒后再试", seconds),
            });
